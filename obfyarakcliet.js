  ...
  init() {
    ...
    const patchChatPanel = () => {
      ...
      try {
        ...
        ChatPanel.prototype.getEmotesCapacity = function () {
          ...
          try {
            return num == null || isNaN(num)
              ? DEFAULT_CAPACITY
              : Math.trunc(
                  Math.min(Math.max(1, num), MAX_CAPACITY)
                ) || DEFAULT_CAPACITY;
          } catch {
        ...
        const currentCapacity =
          parseInt(localStorage.getItem(STORAGE_KEY)) || DEFAULT_CAPACITY;
    ...
    (function () {
      ...
      document.write = function (...args) {
        count++;
        if (count === 2) {
          log("Second document.write detected, running patch...");
          setTimeout(patchChatPanel, 0); // defer to let DOM update
        }
